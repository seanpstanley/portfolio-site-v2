...
export default function MobileNavbar() {
  ...
  return (
    <nav className="z-[999] lg:hidden fixed top-6 right-6" ref={ref}>
      <AnimatePresence>
        {isOpen && (
          <motion.ul
            layout="size"
            className={`${
              isOpen ? "" : "w-0 h-0"
            } fixed left-6 right-24 flex flex-col gap-3 mb-0 p-3 rounded-3xl nav`}
            exit={{ opacity: 0 }}
          >
            {links.map((link, index) => (
              ...
              >
                <Link
                  className={clsx(
                    "flex justify-center items-center w-full py-5 hover:text-[#6c5cfb] transition dark:text-white/60 dark:hover:text-[#6c5cfb]",
                    {
